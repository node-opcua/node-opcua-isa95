...
exports.install = function(opcua) {
    ...
    opcua.AddressSpace.prototype.addEquipment = function(options) {
        ...
        options.definedByEquipmentClass.forEach(function(equipmentClass){
            ...
            if (equipmentClass.modellingRule === "OptionalPlaceholder") {
                return;
            }
            //xx console.log("---",equipmentClass.modellingRule,equipmentClass.constructor.name.toString(),equipmentClass.browseName.toString());
            assert(equipmentClass.isSupertypeOf(equipmentClassType));
            _addDefinedByEquipmentClassReference(equipment,equipmentClass);
        });
        ...
        options.definedByEquipmentClass.forEach(function(equipmentClass){
            _transferISA95Attributes(equipment,equipmentClass);
        });
    ...
    function _addDefinedByEquipmentClassReference(instance,type) {
        ...
        if (!instance.definedByEquipmentClass) {
            instance.definedByEquipmentClass = [];
        }
        instance.definedByEquipmentClass.push(type);
    }
