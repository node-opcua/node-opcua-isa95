...
module.exports = function (opcua) {
    ...
    function _equipment_madeUpOfEquipments() {
        ...
        const madeUpOfEquipment = addressSpace.findISA95ReferenceType("MadeUpOfEquipment");
    ...
    opcua.AddressSpace.prototype.addEquipmentClassType = function(options) {
        ...
        return namespace.addEquipmentClassType(options);
    };
    opcua.Namespace.prototype.addEquipmentClassType = function(options) {

        const addressSpace = this.addressSpace;
        const equipmentClassType = addressSpace.findISA95ObjectType("EquipmentClassType");
        if (!equipmentClassType){
            throw new Error("cannot find " + "EquipmentClassType");
        }
        const newEquipmentClassType = this.addObjectType({
    ...
    opcua.AddressSpace.prototype.addEquipmentType = function(options) {
        ...
        const addressSpace = this;
        const namespace = addressSpace.getOwnNamespace();
        return namespace.addEquipmentType(options);
    };
    opcua.Namespace.prototype.addEquipmentType = function(options) {

        const addressSpace = this.addressSpace;
        const namespace = this;

        const ns = addressSpace.getISA95Namespace();
        const equipmentType = addressSpace.findObjectType("EquipmentType",ns);
        ...
        const newEquipmentType = namespace.addObjectType({
            browseName: options.browseName,
    ...
    opcua.AddressSpace.prototype.addEquipment = function(options) {
        ...
        const addressSpace = this;
        const namespace = addressSpace.getOwnNamespace();
        return namespace.addEquipment(options);
    };
    opcua.Namespace.prototype.addEquipment = function(options) {
        const addressSpace = this.addressSpace;
        const namespace = this;

    // The TargetNode of this ReferenceType shall be an Object of EquipmentClassType or its subtype.
        const definedByEquipmentClass = addressSpace.findISA95ReferenceType("DefinedByEquipmentClass");
        const equipmentClassType      = addressSpace.findISA95ObjectType("EquipmentClassType");
        ...
        const equipment = namespace.addObject({
        ...
        if (options.typeDefinition) {
            ...
            const objs = options.typeDefinition.findReferencesExAsObject(definedByEquipmentClass);
            //xx options.definedByEquipmentClass = options.definedByEquipmentClass ||[];
            definedStuff  = [].concat(options.definedByEquipmentClass || [],objs);
...
//        assert(options.definedByEquipmentClass," expecting a definedByEquipmentClass options");
        ...
        if (options.definedByEquipmentClass && options.definedByEquipmentClass._postInstantiateFunc) {
            options.definedByEquipmentClass._postInstantiateFunc(equipment,options.definedByEquipmentClass,options);
  //          equipment.install_extra_properties();
        }
        return equipment;
